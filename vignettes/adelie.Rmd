...
The theory and algorithms in this implementation are inspired by @glmnet, @coxnet, @strongrules and @block.
...
problem above. By default we set the weights to $1/n$, and if supplied
they are renormalized to sum to 1. This is equivalent to the weighted sum-of-squared errors
$$
\begin{align*}
    \mbox{RSS}
    &=
...
(see [Examples]), and generally have over 160 billion entries with 30\% non-zero entries.

For these reasons, we found it fruitful to abstract out the matrix class.
`adelie` provides a few matrix classes.
...
saved ourselves the trouble, because it will do so anyway. But we will
...
We currently do not code that allows one to view these matrix objects;
...
method yet, we will use trickery to do
that, by multiplying it by the identity. Since only row-sparse
matrices are allowed, we will make one.
...
standardize them. Lets use that case as an example.
...
In regression settings, we may want to include pairwise interaction terms amongst a subset of features to capture some non-linearity.
    ...
    For further details of their R package `glinternet`, we provide the following reference:
...
We will develop the approach from scratch using `grpnet` as the work
engine, which helps to explain how the model works. Then we
demonstrate the functions that are provided in the `adelie` package
for doing it all automatically.
...
The true model has main effects and interactions between variables 1
and 12.

```{r}
predict(cvfit,type="nonzero")
```
The model has found them. Digging in to see the coefficents is less
helpful. 
...
- For a quantitative and a factor variable, it does the same, but with
the quantitative feature multiplied into each of the columns of the
one-hot matrix. The one-hot matrix is also included, so if the
quntitative feature has 5 levels, the result here is a 10-column matrix.
- For two qualitative features we get the Hadamard product of their
...
Now, we construct the full feature matrix including the one-hot
 ...
 Note that we have already standardized the quantittative features in
